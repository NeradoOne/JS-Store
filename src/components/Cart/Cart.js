...
const Cart = () => {
  ...
  return (
    <main className='container mx-auto'>
      <Link className="btn btn-circle btn-ghost text-secondary btn-xs" to={'/products'}>
      ...
      <div className='max-w-xl grid grid-cols-1'>
        {cartList.map(i => (
          <NavLink className={'mt-5 bg-base-300 shadow rounded p-4 my-2'} to={`/products/${i.category}/${i.id}`} key={i.id}>
            <section className='mx-auto flex justify-between items-center'  >
              <img className='h-12 w-12 rounded-full' src={i.image} />
              <div className='ml-2' >
