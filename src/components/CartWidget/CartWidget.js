      ...
      <div className="dropdown dropdown-end">
        <label tabIndex={0} className="btn btn-ghost text-primary btn-circle">
          <div className="indicator">
            ...
            <span className={totalItems ? "badge badge-sm bg-secondary indicator-item" : 'hidden'}>{totalItems ? totalItems : ''}</span>
          </div>
        </label>
        <div tabIndex={0} className="mt-3 card card-compact dropdown-content bg-base-300 shadow">
          <div className="card-body">
            ...
            {cartList.map(i => (
              <section className='flex justify-between rounded p-2 my-2 items-center w-72 bg-white' key={i.id} >
                <NavLink to={`/products/${i.category}/${i.id}`} >
                  <img className='h-12 rounded-full' src={i.image} alt={i.title} />
                  <div className='ml-2' >
